...
def cellpose_file_loader(maskpath, image_size=None, completeload=False):
    ...
    # Keep masks boolean to avoid dtype-conversion warnings downstream when saving.
    output = np.zeros((masks.shape[0], masks.shape[1], len(mask_idxs)), dtype=bool)
        ...
        for file in files:
            if file.endswith('.tif'):
                ...
                for mask_idx in mask_idxs:
                    # Boolean mask avoids repeated skimage dtype warnings (int64 -> uint8).
                    single_cell_mask = (masks == mask_idx)
