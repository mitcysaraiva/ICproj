...
def define_model(mode=None, size_target=None, class_count=None, initial_lr=None, opt=None, init_source=None, dropout_rate=0.0):
    ...
    elif mode == 'EfficientNetB0':
        if EfficientNetB0 is None:
            raise ImportError('EfficientNetB0 is not available in this Keras build.')
        base = EfficientNetB0(include_top=False, weights=weights, input_shape=size_target, pooling='avg')
        x = base.output
...
def train(mode=None, X_train=None, y_train=None, size_target=None, pad_cells=False, resize_cells=False, class_count=None, logdir=None, verbose=False, **kwargs):
    ...
    val_seed = kwargs.get('val_seed', None)
    early_stopping_patience = kwargs.get('early_stopping_patience', None)
    use_lr_scheduler = kwargs.get('use_lr_scheduler', True)
    plot_diagnostics = kwargs.get('plot_diagnostics', True)
    trial = kwargs.get('trial', None)
    prune_monitor = kwargs.get('prune_monitor', 'val_accuracy')
    hist_eq = kwargs.get('hist_eq', True)
    ...
    def _wrap_preprocess(preprocess_fn):
        def _fn(img):
            ...
            if x.ndim == 3:
                x = preprocess_fn(np.expand_dims(x, 0))
                return x[0]
            return preprocess_fn(x)
        return _fn

    preprocess_fn = None
    if mode in {'EfficientNetB0'}:
        if efficientnet_preprocess_input is not None:
            preprocess_fn = _wrap_preprocess(efficientnet_preprocess_input)
    ...
    traingen = DataGenerator(train_X, train_y,
                 batch_size=batch_size, shuffle=True, augment=augment, aug1=seq1, aug2=seq2, preprocess_fn=preprocess_fn)
    ...
    if trial is not None:
        try:
            ...
            raise RuntimeError('Optuna is required when passing trial=...') from e

        class _OptunaPruningCallback(keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs=None):
                ...
                if value is None and self.monitor == 'val_accuracy':
                    value = logs.get('val_acc')
                if value is None:
                    return
                self.trial.report(float(value), step=int(epoch))
                if self.trial.should_prune():
                    raise optuna.TrialPruned(f'Pruned at epoch {epoch}, {self.monitor}={value}')

        callbacks.append(_OptunaPruningCallback(trial, prune_monitor))
    ...
    if return_best:
        metric_values = history.history.get(return_best_monitor)
        if metric_values is None and return_best_monitor == 'val_accuracy':
            metric_values = history.history.get('val_acc')
        if metric_values is None:
            return None
        if 'loss' in return_best_monitor:
            return float(np.min(metric_values))
        return float(np.max(metric_values))
