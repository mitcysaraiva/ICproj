...
def summarize_diagnostics(history, plot_title):
    ...
    axs[1].plot(history.history.get('acc', history.history.get('accuracy')), color='blue', label='train')
    axs[1].plot(history.history.get('val_acc', history.history.get('val_accuracy')), color='orange', label='validation')
    axs[1].set_xlabel('Epoch')
...
def inspect_model_data(X, y, n):
    ...
    for i, unique_y in enumerate(uniques):
        ...
        # Index with n, but clamp to available samples for small datasets
        selection = idx[np.clip(n, 0, len(idx) - 1)]
