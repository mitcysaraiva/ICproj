...
def _find_best_checkpoint(logdir: str) -> str | None:
    """Return the most recently modified .h5 checkpoint file under `logdir`, or None."""
    p = Path(logdir)
    if not p.is_dir():
        return None
    h5_files = sorted(p.glob('*.h5'), key=lambda f: f.stat().st_mtime)
    if not h5_files:
        return None
...
best_params = None
_best_study_state = {"best_val": float("-inf")}
if DO_OPTUNA:
    import optuna
    ...
    def objective(trial):
        ...
        # Update best-study model checkpoint if this trial improved validation accuracy.
        ckpt_path = _find_best_checkpoint(trial_logdir)
        if ckpt_path is not None and score is not None:
            try:
                ...
                if current_val > _best_study_state["best_val"]:
                    ...
                    best_dst = os.path.join(TUNE_DIR, 'best_study_model.h5')
...
try:
    final_ckpt = _find_best_checkpoint(LOG_DIR)
    if final_ckpt is not None:
        best_retrain_dst = os.path.join(LOG_DIR, 'best_retrain_model.h5')
        shutil.copy2(final_ckpt, best_retrain_dst)
        print('Saved best retrain model to:', best_retrain_dst)
    else:
        print('WARNING: no .h5 checkpoint found in final LOG_DIR:', LOG_DIR)
except Exception as e:
